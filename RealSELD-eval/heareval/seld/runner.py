...
_PRESETS = {
    ...
    # heareval's frame-wise probe and its randomized grid search.
    "mlp": MLP_PARAM_GRID,
}
...
@click.option(
    ...
    "'mlp' is heareval's frame-wise probe with its own PARAM_GRID, selected by "
    "randomized grid search - use --grid-points 8 or more with it. "
    "(Default: default)",
)
@click.option(
...
# ---- head architecture ---------------------------------------------------- #
@click.option(
    "--branch", default=None, type=click.Choice(["rnn", "mlp"]),
    help="Evaluation head. 'rnn' is the BiGRU + self-attention sequence head; "
    "'mlp' is heareval's per-frame FullyConnectedPrediction, which has no "
    "temporal context and is the control the sequence head has to beat.",
)
@click.option("--hidden-layers", default=None, type=click.INT,
              help="MLP branch: number of hidden layers.")

...
class AudioSphere(pl.LightningModule):
    """
    ...
    def get_audio_representation_from_layer(self, x, strategy="mean", block_num=-1):
        ...
        if strategy == "mean":
            ...
            raise ValueError(f"Strategy '{strategy}' is unrecognized.")
    def get_all_layer_representations(self, x, strategy="raw"):
        """
        HuggingFace-style hidden-state extraction. Returns the patch-embed (conv)
        output as layer 0 — analogous to SUPERB's CNN feature extractor output —
        followed by the output of every transformer block. Total layers for "base"
        is 1 + 12 = 13.

        Returns
        -------
        strategy="raw":  Tensor (B, L, T, F*D)
        strategy="mean": Tensor (B, L, D)
        """
        B = x.shape[0]
        x = x.transpose(2, 3)
        x = self.spectrogram_normalize(x)
        ...
        patches = patches.flatten(2)
        encoded_patches = self.patch_strategy.embed(x, self.patch_embed)
        # encoded_patches: (B, num_patches, D), no pos-embed, no cls

        mask = torch.zeros((B, self.num_patches), dtype=torch.bool, device=self.device)
        non_mask_index = ~mask

        # Layer 0: conv (patch-embed) output, SUPERB-style.
        hidden_states = [encoded_patches]

        # Replicate pass_through_encoder, capturing each block output.
        h = encoded_patches + self.encoder.pos_embedding[:, self.encoder_cls_token_num:, :]
        h = h[non_mask_index, :].reshape((B, -1, h.shape[-1]))
        cls_token = (
            self.cls_token.expand(B, -1, -1)
            + self.encoder.pos_embedding[:, :1, :]
        )
        try:
            ...
            h = torch.cat((cls_token, dist_token, h), dim=1)
        except Exception:
        ...
        for block in self.encoder.layers:
            h = block(h)
            # Drop cls (and dist if present) before storing — keep only patch tokens.
            hidden_states.append(h[:, self.encoder_cls_token_num:, :])

        # Stack: (B, L, num_patches, D)
        hidden_states = torch.stack(hidden_states, dim=1)

        if strategy == "raw":
            f, _ = self.grid_size
            return rearrange(
                hidden_states, "b l (f t) d -> b l t (f d)",

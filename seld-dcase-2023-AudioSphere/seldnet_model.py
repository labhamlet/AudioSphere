...
class AudioSphereSELD(nn.Module):
    ...
    --------
        log_mel (B, 7, T_in, F_mel)   ambisonic log-mel + IVs (from the extractor)
          |
          v  AudioSphere raw output          audio_sphere.encode(log_mel)
        (B, T, F*D)
          |
    ...
    def __init__(self, out_shape, params,
        ...
        if audio_sphere is None:
            ...
            audio_sphere = RuntimeAudioSphere(
                ...
                layer=layer,
                use_mwmae_decoder=runtime_kwargs.pop("use_mwmae_decoder", False),
    ...
    def forward(self, log_mel):
        ...
        # RuntimeAudioSphere.encode handles chunking to input_tdim, padding,
        # and runs the encoder under torch.no_grad().
        z = self.audio_sphere.encode(log_mel)          # (B, T, F*D)
